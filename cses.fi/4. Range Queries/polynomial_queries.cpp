...
struct LazyST
{
    ...
    {
        ll offset, slope;
        Update(ll o, ll s) { offset = o; slope = s; }
    ...
    {
        ll real_off = upd.offset + l * upd.slope;
        a += real_off * (r - l + 1);
        a += (r - l + 1) * 1LL * (r - l + 2) / 2 * upd.slope;
    }
    Update combine_updates(Update prev, Update upd)
    {
        Update res(0,0);
        res.offset = prev.offset + upd.offset;
        res.slope = prev.slope + upd.slope;
        return res;
    }
...
int main()
{
    ...
    while(q--)
    {
        ...
        if(t == 1) st.update(a, b, LazyST::Update(-a, 1));
